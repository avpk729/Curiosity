...
Write 2-4 paragraphs of flowing prose. No bullet points, no markdown. At the very end, add a signpost sentence inviting the reader to go deeper: [SIGNPOST: your bridging sentence here].`,

  masters:`You are writing a graduate-level treatment of a specific section. The student has read 5th-grade and undergraduate explanations of this section. Begin with 2-3 sentences anchoring the key theoretical framework or insight that elevates this from undergraduate understanding, then go deep. Use full technical rigour: advanced mechanisms, theoretical frameworks, mathematical formulations (LaTeX: $...$ inline, $$...$$ block), research context.

Mark prerequisites: [PREREQ: concept name]. Mark frontier extensions: [DEEPER: concept name]. Use each 2-4 times.

Write 3-5 paragraphs. At the very end, add a signpost sentence: [SIGNPOST: your bridging sentence here].`,

  phd:`You are writing a doctoral-level treatment of a specific section. The student has read all lower-level explanations of this section. Open with 2-3 sentences anchoring the section's position in the research literature and its central open question. Focus on research frontiers, open problems, competing frameworks, mathematical rigour (LaTeX: $...$ inline, $$...$$ block), seminal and recent literature, methodological debates.
...
Write a thorough explanation. For grade5 level: start with [OVERVIEW: ...] then 3-5 sections each beginning with [HEADING: Title].
For other levels: write flowing prose paragraphs (no section headings needed — this is a deep-dive into one section).
No bullet points, no markdown, no bold markers.
...
You are writing the ${LEVEL_LABELS_LOCAL[level]} level deep-dive for ONE specific section.

Parent topic: "${topic}"
Section heading: "${sectionHeading}"

Write a flowing, thorough explanation of "${sectionHeading}" at ${LEVEL_LABELS_LOCAL[level]} level.
No [HEADING:] tags needed — this is one focused section.
No bullet points, no markdown. Pure prose paragraphs.
Include LaTeX math where genuinely useful ($...$ inline, $$...$$ block).
Cite sources inline as [1], [2] etc.
End with [SIGNPOST: one sentence inviting the reader to go one level deeper] — except at PhD level.
...
Bibliography: real verifiable sources only. 2-4 sources.`;
}
...
// ── Explore Section (inline dig-deeper) ───────────────
app.post("/api/explore-section", async (req, res) => {
  ...
  const lvl = level;
  // Use section heading as the "branch" key for caching
  const branchKey = `__section__${sectionHeading.trim()}`;

  const cached = db.prepare("SELECT content,citations FROM explanations WHERE topic_lower=? AND branch IS ? AND level=?")
    .get(topicLower, branchKey, lvl);

  let result;
  if (cached) {
    result = { content: cached.content, bibliography: JSON.parse(cached.citations || "[]") };
  } else {
    try {
      const response = await fetch("https://api.anthropic.com/v1/messages", {
        method: "POST",
        ...
        body: JSON.stringify({
          ...
          messages: [{ role: "user", content: buildSectionPrompt(topic.trim(), sectionHeading.trim(), lvl, prevLevelContent) }]
        })
      });
      const data = await response.json();
      if (data.error) throw new Error(data.error.message);
      ...
      let content = raw;
      if (metaMatch) {
        try { const json = JSON.parse(metaMatch[1].trim()); bibliography = json.bibliography || []; } catch(e) {}
        content = raw.replace(/<META>[\s\S]*?<\/META>/, "").trim();
      }
      db.prepare(`INSERT OR REPLACE INTO explanations (topic_lower,branch,level,content,branches,citations,visuals,created_at) VALUES (?,?,?,?,?,?,?,?)`)
        .run(topicLower, branchKey, lvl, content, "[]", JSON.stringify(bibliography), "{}", Date.now());
      result = { content, bibliography };
    } catch (err) {
      console.error("Section explore error:", err.message);
      return res.status(500).json({ error: "Failed to fetch section explanation. Please try again." });
    }
  }
  res.json(result);
});
